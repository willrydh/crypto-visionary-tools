...
export const TransparentWhiteButton = React.forwardRef<HTMLButtonElement, TransparentWhiteButtonProps>(
  ({ className, children, ...props }, ref) => (
    <button
      ref={ref}
      className={cn(
        "bg-transparent border border-border text-primary-foreground rounded-lg px-6 py-2 font-semibold transition-colors duration-150 hover:bg-card/10 active:bg-card/20 focus:outline-none focus:ring-2 focus:ring-ring/70 whitespace-nowrap",
        className
      )}
      {...props}
    >
